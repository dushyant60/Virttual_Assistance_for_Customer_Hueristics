...
app.post("/voice/callback", (req, res) => {  
    ...
    twilioClient.calls.create({  
        ...
        console.info('Call initiated:', call.sid); // 📢
...
app.delete("/voice/waiting-list/:number", (req, res) => {  
    ...
    if (index !== -1) {  
        ...
        console.info(`Number ${number} removed from the waiting list.`); // 📢
        res.json({ message: `Number ${number} removed from the waiting list.` });  
    } else {  
        console.warn(`Number ${number} not found in the waiting list.`); // ⚠️
...
async function uploadToBlobStorage(filePath, timestamp) {  
    ...
    try {  
        ...
        console.info('File uploaded to Azure Blob Storage:', response); // 📢
        return response;  
    } catch (error) {  
...
async function geturl(url) {  
    try {  
        ...
        console.info('Audio file downloaded successfully'); // 📢
        const timestamp = Date.now();  
        const uploadResponse = await uploadToBlobStorage('audio.wav', timestamp);  
        console.info('Audio file uploaded to Azure Blob Storage'); // 📢
...
wss.on("connection", function connection(ws) {  
    ...
    ws.on("message", async function incoming(message) {  
        ...
        switch (msg.event) {  
            case "connected":  
                console.info(`A new call has connected.`); // 📢
                break;  
            case "start":  
                ...
                conversationTranscriber.sessionStarted = (s, e) => {  
                    console.info(`Session started with session ID: ${e.sessionId}`); // 📢
                ...
                conversationTranscriber.transcribed = (s, e) => {  
                    if (e.result.reason === sdk.ResultReason.RecognizedSpeech) {  
                        console.info("TRANSCRIBED: Text=" + e.result.text + " Speaker ID=" + e.result.speakerId); // 📢
                ...
                conversationTranscriber.canceled = (s, e) => {  
                    ...
                    if (e.reason == sdk.CancellationReason.Error) {  
                        ...
                        console.error("CANCELED: Did you update the key and location/region info?"); // ❌
                    }  
                ...
                conversationTranscriber.startTranscribingAsync(() => {  
                    console.info("Continuous Transcription Started"); // 📢
                }, err => {  
            ...
            case "media":  
                if (msg.media && msg.media.payload) {  
                    ...
                    console.warn("Received undefined audio payload."); // ⚠️
                }  
                break;  
            case "stop":  
                ...
                console.info(`PCM audio data saved to ${wavFilePath}`); // 📢
...
server.listen(3001, () => console.info("Express server is running on localhost:3001")); // 📢